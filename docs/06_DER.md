...
| **snr_low** | BOOLEAN | S |  | TRUE si la terminal reporta señal persistentemente baja. Derivado como `not isSnrAboveNoiseFloor` (confirmado contra la antena real el 12/08/2026; `isSnrPersistentlyLow`, asumido originalmente, no existe en este firmware). Reemplaza `snr_db` desde schema_version 1.1 — el firmware real no expone SNR numérico (ADR-17). NULL si la API interna no está accesible. | false | RF-03 |
| **is_obstructed** | BOOLEAN | S |  | TRUE si `dishGetDiagnostics.alerts.obstructed` está presente y en TRUE (**corregido 20/08/2026**: la fila anterior de este documento usaba `obstructionStats.fractionObstructed > 0` de `get_status`, pero ese campo es una fracción ACUMULADA desde que arrancó la ventana de validación de la antena — horas/días de uptime —, no el estado actual; con ese umbral daba TRUE casi siempre una vez que hubo cualquier obstrucción alguna vez, confirmado con datos reales: 8358/8359 muestras en 6 días con apenas 0.04% acumulado). `alerts` usa serialización JSON dispersa (proto3): solo aparecen las claves en TRUE, así que su ausencia significa FALSE. NULL solo si `get_diagnostics` no respondió ese ciclo (antena no disponible momentáneamente). | false | RF-03 |
...
| **handover_count** | SMALLINT | S |  | Cantidad de eventos de handover satelital (`didSwitch=true`) detectados desde la medición anterior. Obtenido de `get_history.dishGetHistory.outages[]` del endpoint gRPC interno (ADR-16). 0 es el valor normal (sin handovers en el intervalo); NULL solo si la medición falló. | 0 | RF-03 |
| **outage_duration_ms** | FLOAT8 | S |  | Milisegundos totales de corte asociados a los handovers de `handover_count`, en el mismo intervalo. Suma de `durationNs` de los outages correspondientes, convertida a ms (ADR-16). 0.0 es el valor normal; NULL solo si la medición falló. | 0.0 | RF-03 |
| **tilt_angle_deg** | FLOAT8 | S |  | Inclinación física de la antena, en grados. Obtenido de `dishGetStatus.alignmentStats.tiltAngleDeg` (ADR-18). NULL si no disponible. | 2.1 | RF-03 |
| **boresight_azimuth_deg** | FLOAT8 | S |  | Azimuth real de apuntamiento, en grados, rango firmado -180..180 (confirmado contra la antena real el 12/08/2026; la convención de brújula 0-360 asumida originalmente no coincide con el firmware). `alignmentStats.boresightAzimuthDeg` (ADR-18). NULL si no disponible. | -175.7 | RF-03 |
| **boresight_elevation_deg** | FLOAT8 | S |  | Elevación real de apuntamiento, en grados. `alignmentStats.boresightElevationDeg` (ADR-18). NULL si no disponible. | 51.7 | RF-03 |
| **desired_boresight_azimuth_deg** | FLOAT8 | S |  | Azimuth objetivo calculado por la antena, en grados, rango firmado -180..180 (ver boresight_azimuth_deg). `alignmentStats.desiredBoresightAzimuthDeg` (ADR-18). NULL si no disponible. | -176.0 | RF-03 |
| **desired_boresight_elevation_deg** | FLOAT8 | S |  | Elevación objetivo calculada por la antena, en grados. `alignmentStats.desiredBoresightElevationDeg` (ADR-18). NULL si no disponible. | 52.0 | RF-03 |
| **attitude_uncertainty_deg** | FLOAT8 | S |  | Incertidumbre del algoritmo de estimación de actitud, en grados. `alignmentStats.attitudeUncertaintyDeg` (ADR-18). NULL si no disponible. | 0.3 | RF-03 |
| **schema_version** | VARCHAR(16) | N |  | Versión del esquema JSON del paquete de origen. Permite detectar incompatibilidades entre versiones del script y del consumer. Valor actual: '1.1'. | 1.1 | RF-04 |
...
| -- Hypertable: particionado por día SELECT create_hypertable('network_metrics', 'time',     chunk_time_interval => INTERVAL '1 day',     if_not_exists       => TRUE); -- Compresión columnar: chunks > 7 días ALTER TABLE network_metrics SET (     timescaledb.compress,     timescaledb.compress_segmentby = 'node_id',   -- agrupa por nodo para mejor ratio     timescaledb.compress_orderby   = 'time DESC'  -- ordena por tiempo dentro del segmento ); SELECT add_compression_policy('network_metrics', INTERVAL '7 days'); -- Retención: elimina chunks crudos > 6 meses SELECT add_retention_policy('network_metrics', INTERVAL '6 months'); -- CHECK constraints ALTER TABLE network_metrics     ADD CONSTRAINT chk_netmet_loss         CHECK (packet_loss_pct IS NULL OR (packet_loss_pct >= 0 AND packet_loss_pct <= 100)),     ADD CONSTRAINT chk_netmet_throughput_down         CHECK (throughput_down_bps IS NULL OR throughput_down_bps >= 0),     ADD CONSTRAINT chk_netmet_throughput_up         CHECK (throughput_up_bps IS NULL OR throughput_up_bps >= 0),     ADD CONSTRAINT chk_netmet_handover_count         CHECK (handover_count IS NULL OR handover_count >= 0),     ADD CONSTRAINT chk_netmet_outage_duration         CHECK (outage_duration_ms IS NULL OR outage_duration_ms >= 0),     ADD CONSTRAINT chk_netmet_tilt         CHECK (tilt_angle_deg IS NULL OR (tilt_angle_deg >= 0 AND tilt_angle_deg <= 90)),     ADD CONSTRAINT chk_netmet_boresight_azimuth         CHECK (boresight_azimuth_deg IS NULL OR (boresight_azimuth_deg >= -180 AND boresight_azimuth_deg <= 180)),     ADD CONSTRAINT chk_netmet_boresight_elevation         CHECK (boresight_elevation_deg IS NULL OR (boresight_elevation_deg >= 0 AND boresight_elevation_deg <= 90)),     ADD CONSTRAINT chk_netmet_desired_azimuth         CHECK (desired_boresight_azimuth_deg IS NULL OR (desired_boresight_azimuth_deg >= -180 AND desired_boresight_azimuth_deg <= 180)),     ADD CONSTRAINT chk_netmet_desired_elevation         CHECK (desired_boresight_elevation_deg IS NULL OR (desired_boresight_elevation_deg >= 0 AND desired_boresight_elevation_deg <= 90)),     ADD CONSTRAINT chk_netmet_attitude_uncertainty         CHECK (attitude_uncertainty_deg IS NULL OR attitude_uncertainty_deg >= 0); |
| --- |
...
| **sum_handover_count** | BIGINT | S | CAGG | SUM(handover_count) — total de handovers en la hora (ADR-16). | 4 | RNF-02 |
| **sum_outage_duration_ms** | FLOAT8 | S | CAGG | SUM(outage_duration_ms) — corte total acumulado en la hora, en ms (ADR-16). | 1840.0 | RNF-02 |
| **avg_tilt_angle_deg** | FLOAT8 | S | CAGG | AVG(tilt_angle_deg) — inclinación media de la antena en la hora (ADR-18). | 2.3 | RNF-02 |
...
| **sum_handover_count** | BIGINT | S | CAGG | SUM(handover_count) — total de handovers en el día (ADR-16). | 62 | RNF-02 |
| **sum_outage_duration_ms** | FLOAT8 | S | CAGG | SUM(outage_duration_ms) — corte total acumulado en el día, en ms (ADR-16). | 27400.0 | RNF-02 |
| **avg_tilt_angle_deg** | FLOAT8 | S | CAGG | AVG(tilt_angle_deg) — inclinación media de la antena en el día (ADR-18). | 2.2 | RNF-02 |
...
| **source** | VARCHAR(32) | N | PK IDX | Origen del dato. Valores permitidos: 'local_sensor' (BME280 físico o mock), 'api_open_meteo' (Open-Meteo API), 'api_owm' (OpenWeatherMap), 'api_smn' (SMN Argentina), 'api_omixom' (Omixom/OHMC, Laboratorio de Hidráulica-UNC — enmienda 12/09/2026, integrador implementado contra `estaciones-beta.ohmc.com.ar`). CHECK constraint obligatorio. | local_sensor | RF-12, RF-21 |
