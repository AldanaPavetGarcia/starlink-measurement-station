...
| **snr_low** | BOOLEAN | S |  | TRUE si la terminal reporta señal persistentemente baja (`isSnrPersistentlyLow`). Reemplaza `snr_db` desde schema_version 1.1 — el firmware real no expone SNR numérico (ADR-17). NULL si la API interna no está accesible. | false | RF-03 |
...
| **boresight_azimuth_deg** | FLOAT8 | S |  | Azimuth real de apuntamiento, en grados. `alignmentStats.boresightAzimuthDeg` (ADR-18). NULL si no disponible. | 184.3 | RF-03 |
...
| **desired_boresight_azimuth_deg** | FLOAT8 | S |  | Azimuth objetivo calculado por la antena, en grados. `alignmentStats.desiredBoresightAzimuthDeg` (ADR-18). NULL si no disponible. | 184.0 | RF-03 |
